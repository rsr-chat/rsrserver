...
impl CommandHandler<state::Anonymous> for Info {
...
impl CommandHandler<state::Registered> for Info {
    ...
    async fn handle<'a, S: Storage>(
        ...
        ctx.send_client_unchecked(&format!(":* 374 {nick}  :End of INFO list\r\n")).await?;
        Ok(ctx)
    }
}

impl CommandHandler<state::Authenticated> for Info {
